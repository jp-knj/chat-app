...
export const Input = styled.input`
  ...
  border: ${({ theme }) =>
    theme.isLight ? theme.colors.gray.medium + ' .1em solid' : 'none'};
  width: 100%;
  box-shadow: 0 1px 8px
    ${({ theme }) => (theme.isLight ? 'rgb(0,0,0,0)' : 'rgba(24,24,24,20%)')};
  &:focus {
    outline: none;
    border: 0.1rem solid
      ${({ theme }) =>
        theme.isLight ? theme.colors.purple.light : theme.colors.white.light};
  }
`;
